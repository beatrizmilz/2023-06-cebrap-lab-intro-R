...
# ctrl + enter
# selecionar + run
# colocar o cursor no trecho de código + run
```
...
# usar apenas parenteses para delimitar a ordem das operações
```
...
```{r}
estuda <- TRUE
# VERDADEIRO É TRUE (com letras maiúsculas)
# FALSO É FALSE
```


2.  Tente adivinhar o tipo dos objetos abaixo: texto, número, lógico.
...
```{r}
# selecionar o código + ctrl + shift + a para deixar com espaços certinho
cor_favorita <- "rosa"
```
...
# quando voce for usar além dos primeiros 2 argumentos
# é bom usar o nome
seq(from = 1, to = 10, by = 2)
# descubra o nome na aba HELP pesquisando pela função
```
...
idade_irmaos <- 2023 - ano_nascimento_irmaos
...
primeiro_semestre_arrumado <- c("Mes zero", primeiro_semestre)
...
primeiro_semestre[c(1,3)]

# remove o elemento 1
primeiro_semestre[-1] 

fev_ate_junho <- primeiro_semestre[-1] 
...
# atualizar janeiro para JANEIRO

primeiro_semestre[1] <- "JANEIRO"
...
# REMOVER NA'S PARA FAZER A CONTA
sum(vetor_com_na, na.rm = TRUE)
# min, max, sd, var, mean, median
# Funcoes basicas do R para estatistica descritiva


sum(vetor_com_na, na.rm = FALSE)
```
...
## Tarefa em grupo: (ENVIAR PARA TREINAR NA QUINTA)
...
```{r}
# quero usar a funcao write_bib do pacote knitr
knitr::write_bib()
...
# cria um arquivo que pode ser importado para a biblioteca de refencias
knitr::write_bib(x = c("tidyverse", "abjData"), 
